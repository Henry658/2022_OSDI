...
int initramfs_register(){
	...
	initramfs_v_ops->mkdir = initramfs_v_mkdir;
...
int initramfs_load_dentry(struct dentry* dir, char* component_name) {
	...
	struct initramfs_internal* child_internal = (struct initramfs_internal*)dynamic_alloc(sizeof(struct initramfs_internal));
	child_internal->size = get_usr_program_size(component_name);
	child_internal->p = get_usr_program_address(component_name);
...
int initramfs_v_mkdir(struct vnode *dir, struct vnode **target, const char *component_name) {
