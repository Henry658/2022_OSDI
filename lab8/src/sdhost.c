...
// Ref: https://lexra.pixnet.net/blog/post/303910876-%EF%BF%AD-master-boot-record-(mbr)-%E4%BB%A5%E5%8F%8A-fat32-%E8%A7%A3%E6%9E%90
...
int sd_mount(){
    ...
    // parse first partition only                                        offsets      length      
struct mbr_partition p1;                                              // 000 ~ 1BD     446        code area
    memcpy(&p1, buf + 446, sizeof(struct mbr_partition));             // 1BE ~ 1FD      64
    ...
    if (p1.partition_type == 0x0b) {  // FAT32 with CHS addressing
        ...
        struct fat32_internal* root_internal = (struct fat32_internal*)dynamic_alloc(sizeof(struct fat32_internal));
