...
void timer_list_init(){
	...
	timer_event_list = simple_alloc(sizeof(list_head_t));
...
void core_timer_disable(){
	...
	__asm__ __volatile__("ldr x1, ="XSTR(CORE0_TIMER_IRQ_CTRL));
	__asm__ __volatile__("str w0, [x1]");// unmask timer interrupt
}
