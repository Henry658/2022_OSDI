...
void do_exit(){
	...
	task_struct_t * new_current = current;
	
	new_current = (task_struct_t *) new_current->head.next;
    	if(list_is_head(&new_current->head, &run_queue_list)){
    		new_current = (task_struct_t *) new_current->head.next;
    	}
