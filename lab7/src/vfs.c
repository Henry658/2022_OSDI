...
void rootfs_init(){
	...
	//set initramfs
	vfs_mkdir("/initramfs");
	vfs_mount("initramfs", "/initramfs", "initramfs");
	

	vnode_t * target_dir; // target vnode
	char target_path[128]; //target_path(empty buf)
	traversal("/initramfs", &target_dir, target_path);
	traversal("/initramfs/vfs1.img", &target_dir, target_path);
	...
	*target_node = dentry->vnode;
	...
	list_for_each(pointer, &dentry->childs){//traverse list
		...
		if(!strcmp_2(curr_dentry->name, target_path)){
			...
			else if(curr_dentry->type == REGULAR_FILE){
				traversal_recursive(curr_dentry, path + i, target_node, target_path);
			}
			break;
		}
...
int vfs_mount(const char* device, const char* mount_point, const char* filesystem){
	...
	if(!strcmp(filesystem, "tmpfs")){
		filesystem_t* tmpfs = (filesystem_t*)dynamic_alloc(sizeof(filesystem_t));
		tmpfs->name = (char *)dynamic_alloc(sizeof(char) * strlen(device));
	...
	if(!strcmp(filesystem, "initramfs")){
		...
		initramfs->name = (char *)dynamic_alloc(sizeof(char) * strlen(device));
		strcpy(initramfs->name, device);
...
/*
// error: -1: directory, not a mount point, -3: not found
int vfs_umount(const char* mount_point){
	...
	// unmount
	struct list_head *pointer;
	list_for_each(pointer, &mount_dir->dentry->childs){
		dentry_t* dentry = list_entry(pointer, dentry_t, list);
		list_del_entry(pointer);
		dynamic_free(dentry);
	}
	mount_dir->dentry->mount_parent->mount_point = NULL;
